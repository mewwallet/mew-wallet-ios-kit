...
class EIP712Tests: QuickSpec {
  ...
  private var chainIdAsStringJson: String = {
    return """
    {
      "domain": {
      ...
      "message": {
        ...
        "to": "1",
        "nonce": "12345",
        "data": "0xe71bdf41000000000000000000000000112233445566778899aabbccddeeff0011223344"
      }
    }
    """
  }()
  
  private var chainIdAsStringExpected: TypedMessage = {
    let types: MessageTypes = [
      "EIP712Domain": [
        .init(name: "name", type: "string"),
        .init(name: "version", type: "string"),
  ...
  private func executeTestsSignTypeV1() {
    describe("sign payload v1") {
      it("should hash a message with multiple entries") {
        ...
        do {
          let typedMessage = try TypedMessage(json: typedData, version: .v1)
          let hash = try mew_wallet_ios_kit.hash(message: typedMessage, version: .v1)
          expect(hash.toHexString).to(equal("f7ad23226db5c1c00ca0ca1468fd49c8f8bbc1489bc1c382de5adc557a69c229"))
        } catch {
          fail(error.localizedDescription)
        }
      }
      
      it("should hash a message with multiple entries 2") {
        let typedData: [[String: Any]] = [
          [
            "type": "string",
            "name": "Message",
        ...
        do {
          let typedMessage = try TypedMessage(json: typedData, version: .v1)
          let hash = try mew_wallet_ios_kit.hash(message: typedMessage, version: .v1)
          expect(hash.toHexString).to(equal("7bcdd3ffeab400ef2294ebf69b2defd370bd29ac576c2569b7ea2c48f49ab1ae"))
        } catch {
          fail(error.localizedDescription)
        }
      }
      
      it("should hash a message with incorrect native type of value") {
        let typedData: [[String: Any]] = [
          [
            "type": "string",
            "name": "Message",
  ...
  private func executeTestsSignTypeV3() {
      ...
      it("should encode typed message with v3") {
        do {
          let data = try encodeData(
            ...
            data: self.typedMessage.message[0],
    ...
    describe("structHash v3") {
      it("should correctly hashStruct the message") {
        do {
    ...
    describe("structHash domain v3") {
      it("should correctly hashStruct the domain") {
        do {
          let data = try hashStruct(
    ...
    describe("hash message") {
      it("it should correctly sign the typed message") {
        do {
          let signed = try mew_wallet_ios_kit.hash(message: self.typedMessage, version: .v3)
  ...
  private func executeTestsSignTypeV4() {
    ...
    describe("encode data primary type Person v4") {
      it("it should encode data") {
        do {
          let data = try encodeData(
            primaryType: "Person",
            data: self.typedMessage_v4.message[0]["from"] as! [String: AnyObject],
    ...
    describe("hash struct to Person v4") {
      it("should hash struct with primary type person") {
        do {
          let data = try hashStruct(
            primaryType: "Person",
            data: (self.typedMessage_v4.message[0]["to"] as! [AnyObject]).first as! [String: AnyObject],
            types: self.typedMessage_v4.types,
            version: .v4
          ).toHexString()
      ...
      it("should encode typed message with v4") {
        do {
          let data = try encodeData(
            ...
            data: self.typedMessage_v4.message[0],
            types: self.typedMessage_v4.types,
            version: .v4
          )
          let result = """
    ...
    describe("hash message v4") {
      it("it should correctly sign the typed message") {
        do {
          let signed = try mew_wallet_ios_kit.hash(message: self.typedMessage_v4, version: .v4)
      ...
      it("should encode data") {
        do {
          ...
          let data = try encodeData(
            ...
            data: self.typedMessageWithBytes32_v3.message[0],
      ...
      it("should encode") {
        do {
          ...
          let data = try encodeData(
            ...
            data: self.typedMessageWithBytes_v3.message[0],
  ...
  private func executeChainIdAsString() {
    describe("Chain id as string") {
      it("should be parsed as Int") {
        do {
          let expectedMessage = self.chainIdAsStringExpected
          let expectedEncoded = try encodeData(primaryType: expectedMessage.primaryType,
                                               data: expectedMessage.message[0],
                                               types: expectedMessage.types,
                                               version: .v3)

          let data = self.chainIdAsStringJson.data(using: .utf8)!
          let jsonObject = try JSONSerialization.jsonObject(with: data, options: []) as! [String: Any]
          let message = try TypedMessage(json: jsonObject, version: .v3)
          let messageEncoded = try encodeData(primaryType: message.primaryType,
                                              data: message.message[0],
                                              types: message.types,
                                              version: .v3)
          
          expect(expectedEncoded.toHexString()).to(equal(messageEncoded.toHexString()))
        } catch {
          fail(error.localizedDescription)
        }
      }
