...
internal extension NSTextCheckingResult {
  ...
  private var returnTypeRange: NSRange?   { range(named: .returnType) }
  ...
  func abiFunctionReturnType(in string: String) -> String? { value(of: .returnType, in: string) }
...
extension ABI.Element.Function {
  ...
  private static func parse(parameters: String) -> [ABI.Element.InOut] {
    ...
    for cmp in cmps {
      ...
      let inOut = ABI.Element.InOut(name: name, type: type)
      inOuts.append(inOut)
    }
  ...
  private static func parse(returnType: String) -> [ABI.Element.InOut] {
    return [
      .init(name: "", type: returnType)
    ].compactMap {
      $0
    }
  }
