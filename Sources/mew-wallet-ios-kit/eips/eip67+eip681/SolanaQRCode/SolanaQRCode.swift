...
public struct SolanaQRCode: EIPQRCode & Equatable {
  ...
  public var equatable: EquatableEIPQRCode { .solana(self) }
  ...
  public init?(_ data: Data) {
    guard let val = SolanaQRCodeParser.parse(data) else { return nil }
    self = val
  }
  
  public init?(_ string: String) {
    guard let val = SolanaQRCodeParser.parse(string) else { return nil }
    self = val
  }
}
...
private struct SolanaQRCodeParser {
  static func parse(_ data: Data) -> SolanaQRCode? {
    guard let string = String(data: data, encoding: .utf8) else { return nil }
    return parse(string)
  }

  static func parse(_ string: String) -> SolanaQRCode? {
    guard let encoding = string.removingPercentEncoding,
          let matcher: NSRegularExpression = .solanaQRCode else { return nil }
    ...
    guard let target = match.solanaQRCodeTarget(in: encoding),
          let targetAddress = Address(solanaAddress: target) else { return nil }
    
    return SolanaQRCode(targetAddress)
  }
}
